...
public class Fase extends JPanel implements ActionListener {
	...
	// Imagem da nave
	private Timer timer;
	// Atualizar a tela a cada movimento que o usuário fazer
	...
	public Fase() {
		...
		// Definir tempo de ação, atualizando a tela a cada movimento
	...
	// Gera valores aleatórios dentro do intervalo passado
	...
	public void paint(Graphics g) {
		...
		if (jogo) {
			...
			// Localização da imagem do missil no jogo, e
			...
			// Localização da imagem do inimigo no jogo, e contagem do numero de
		...
		g.dispose();
		// Limpar a imagem anterior para a próxima pintura
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		nave.movimento();
		repaint(); // Repinta a tela a cada atualição, para movimentar as
					// imagens
		colisoes();

	}

	public void colisoes() { // Verificar se ouve algum tipo de colissão durante
	...
	private class Teclado extends KeyAdapter { // Pegar os eventos da tela
		...
		public void keyPressed(KeyEvent e) {
			...
			if (e.getKeyCode() == KeyEvent.VK_ENTER) {
				jogo = true;
				nave = new Nave(); // Reinicia a nave a posição inicial após
									// teclar enter
				inicializarInimigos(getQntInimigos()); // Reinicia os inimigos a
														// poisição
				// inicial após teclar enter
			}
